...
export default function App(){
  ...
  const bootstrapAdmin=async()=>{
    try{
      ...
      setMessage('');
      const result = await supabase.rpc('bootstrap_first_admin');
      if(result.error) throw result.error;
      await load();
      setMessage('Admin-Rolle aktiviert.');
    }catch(error:any){
  ...
  return <div className="shell">
    ...
    <main>
      ...
      {page==='dashboard' ? <><header><div><h1>Dashboard</h1><p>Medilingo Content Workbench</p></div></header><div className="stats"><div className="card"><span>Clinical Cases</span><b>{cases.length}</b></div><div className="card"><span>Kapitel</span><b>{chapters.length}</b></div><div className="card"><span>Case Pools</span><b>{pools.length}</b></div><div className="card"><span>Rolle</span><b>{profile.role}</b></div></div>{profile.role==='viewer'&&<div className="card"><h3>Ersteinrichtung</h3><p>Dies ist der erste Authoring-Account. Aktiviere einmalig die Admin-Rolle.</p><button className="primary" disabled={bootstrapping} onClick={bootstrapAdmin}>{bootstrapping?'Aktiviere…':'Ersten Admin aktivieren'}</button></div>}<div className="card"><h3>Workflow</h3><p>Draft → Validation → Review → Approved → Published</p></div></> : <><header><div><h1>Clinical Cases</h1><p>Cases strukturiert erfassen und Case Pools zuordnen.</p></div><button className="primary" disabled={!canCreate} onClick={()=>setOpen(true)}>+ Neuer Case</button></header><div className="card">{cases.length===0?<div className="empty">Noch keine Cases vorhanden.</div>:<div className="table"><div className="row head"><span>Case</span><span>Titel</span><span>Purpose</span><span>Status</span></div>{cases.map(c=><div className="row" key={c.id}><b>{c.case_code}</b><span>{title(c.case_translations)||'Ohne Titel'}</span><span>{c.purpose}</span><span>{c.status}</span></div>)}</div>}</div></>}
    </main>
